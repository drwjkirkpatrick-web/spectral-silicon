...
@pytest.fixture
def block():
    return SpectralTransformerBlock(channels=32, modes=16)
...
class TestResolutionInvariance:
    def test_forward_different_seq_lens_same_params(self):
        ...
        block = SpectralTransformerBlock(channels=16, modes=8)
    ...
    def test_param_count_same_for_two_instantiations(self):
        ...
        block_a = SpectralTransformerBlock(channels=16, modes=8)
        block_b = SpectralTransformerBlock(channels=16, modes=8)
...
class TestTransformerSlow:
    def test_large_channels(self):
        block = SpectralTransformerBlock(channels=128, modes=32)
