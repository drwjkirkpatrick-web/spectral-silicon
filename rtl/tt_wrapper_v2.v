...
//==============================================================================
// tt_wrapper_v2.v — Tiny Tapeout Wrapper v2 for Spectral Silicon
//==============================================================================
// Wraps the spectral_mixer_v2 top-level module in the Tiny Tapeout (TT)
// template.  This is the v2 wrapper that uses the improved spectral_mixer_v2
// (shared FFT/IFFT engine, merged activation, serialized channels).
//
// Module name: tt_um_spectral_silicon_v2
//
// Tiny Tapeout pin mapping (identical to v1):
...
//
// SPI-like protocol over ui_in[7:0] (identical to v1):
//   - ui_in[7]    = SCK  (clock)
...
//==============================================================================
module tt_um_spectral_silicon_v2 (
    ...
    reg [2:0]  spi_state;
    reg [1:0]  byte_cnt;           // Which byte (0..3 for 32-bit data)
    ...
    // Uses the improved spectral_mixer_v2 with:
    //   - Shared FFT/IFFT engine (Improvement 1)
    //   - Merged soft-threshold + modReLU (Improvement 8)
    //   - Serialized channel processing (Improvement 9)
    //   - Clock gating and operand isolation (Improvements 4, 10)
    //----------------------------------------------------------------------
    spectral_mixer_v2 #(
        .N(256),
        .D(64),
        ...
        if (!rst_n) begin
            ...
            S_RD_DATA: begin
                if (spi_cs_n) begin
                    ...
                    // On rising edge: shift out next bit on MISO
